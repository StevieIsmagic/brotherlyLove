...
admin.initializeApp({
  credential: admin.credential.cert(require("./brotherlylove.json"))
});
...
exports.storeImage = functions.https.onRequest((request, response) => {
  return cors(request, response, () => {
    ...
    admin.auth().verifyIdToken(idToken)
      .then(decodedToken => {
        console.log("Firebase Cloud F(x) decodedToken: ", decodedToken);
    ...
    .catch(error => {
      ...
      response.status(403).json({ error: "Cloud f(x) Unauthorized!" });
    })
  });
});
